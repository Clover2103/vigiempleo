...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['comp_rl_pn']) && $_FILES['comp_rl_pn']['error'] === UPLOAD_ERR_OK) {
        ...
        $extension = pathinfo($_FILES['comp_rl_pn']["name"], PATHINFO_EXTENSION);
        $file_name = $usuario . '-ref_lab.' . $extension;
        ...
        try {
            $query =("INSERT INTO ref_laboral (numero_doc,nom_empresa,cod_cargo,cargo,tie_ingreso,sig_trab,tie_salida,jefe_inmediato,tele_jefe,comp_lab)
                VALUES (:num_doc,:nom_emp,:tipo_cargo,:cargo,:tie_ing_exp,:auntrabaja,:tie_sali_exp,:jefe,:tel_jefe,:comp_lab)");
        ...
        try {
            $query =("INSERT INTO ref_laboral (numero_doc,nom_empresa,cod_cargo,cargo,tie_ingreso,sig_trab,tie_salida,jefe_inmediato,tele_jefe)
                VALUES (:num_doc,:nom_emp,:tipo_cargo,:cargo,:tie_ing_exp,:auntrabaja,:tie_sali_exp,:jefe,:tel_jefe)");
            $insert = $conexion->prepare("$query");
            $insert -> bindParam(':num_doc',$usuario,PDO::PARAM_STR);
            $insert -> bindParam(':nom_emp',$nombre,PDO::PARAM_STR);
            $insert -> bindParam(':cargo',$cargo,PDO::PARAM_STR);
