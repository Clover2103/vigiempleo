...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $titulo     = $_POST["tit_op_otro_pn"];
    $tiempo     = $_POST["tie_fin_otro_1_pn"];
    $tipo       = $_POST["val_otro_1_pn"];

    if (isset($_FILES['doc_otro_1_pn']) && $_FILES['doc_otro_1_pn']["error"] === 0) {
        ...
        $file_name = $_FILES['doc_otro_1_pn']["name"];
        $extension = pathinfo($file_name, PATHINFO_EXTENSION);
        $file_name = $usuario . '-otro1.' . $extension;
        ...
        if (move_uploaded_file($_FILES['doc_otro_1_pn']["tmp_name"], $add)) {
        ...
        try {
            $query = "UPDATE otros SET nom_inst = :nom_inst, tit_obtenido = :titulo, tiempo_fin = :tiempo, competencia = :competencia
                WHERE numero_doc = :usuario AND id_estu_otro = :tipo";
            $update = $conexion->prepare($query);
            $update->bindParam(':nom_inst', $nom_inst);
            $update->bindParam(':titulo', $titulo);
